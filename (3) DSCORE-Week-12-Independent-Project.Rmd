...
### Columns contained in our dataframe
```{r}
columns(audience_df)
```
...
head(audience_df3)
```
So far so good! Let's try to separate the time as well.

```{r}
audience_df3$Time <- as.Time((audience_df3$Timestamp) ,format = "%H:%M:%S")

head(audience_df3)
```
Not working, we'll circle back to this if necessary. As Sam said, optimize to ge the job  done ASAP. Refinement baadaye. Mustn't. Waste. Time.
...
I still don't have the output that I want, so I'll now attempt to group EVEYRTHING by the *Clicked.On.Ad* feature

```{r}
library(dplyr); library(reshape2)
Summary <- audience_df3
  group_by(Clicked.on.Ad)
  #summarise(Net = sum('Clicked.on.Ad'))
```
